# Version info: R 3.2.3, Biobase 2.30.0, GEOquery 2.40.0
# CLEAR & CLEAN -----------------------------------------------------------
...
# load("../plots/bubble.data.Rdata")
library(ggplot2);
library(readxl)
# both_r <- read_excel("/media/Storage_HDD/C3M/RNAseq/projet_cg_hsg/Cmap/new/both_r.xlsx",
#                      col_names = FALSE)
# df=data.frame(both_r[-c(12,13),])
# df=df[order(df$...3),]
# names(df)[1]="Molecule"
# names(df)[3]="Enrichment"
# df
# ggplot(df,aes(x=factor(Molecule,levels = df$Molecule),y=df$...2,fill=Enrichment))+geom_bar(stat = 'identity')+
# theme(axis.text.x = element_text(size=10, angle=25,colour = c(rep(1,11),2,rep(1,4),2,1))) +labs(title="Top HL60 reversing molecules",x ="", y = "Count")
#

wt_ST3_both <- read_excel("/media/Storage_HDD/C3M/RNAseq/projet_cg_hsg/Cmap/new/wt_ST3_both.xls",
                          sheet = "by cmap name and cell line")
# View(wt_ST3_both)
HL60=wt_ST3_both[which(wt_ST3_both$...3=="HL60"), ]
MCF7=wt_ST3_both[which(wt_ST3_both$...3=="MCF7"), ]
PC3=wt_ST3_both[which(wt_ST3_both$...3=="PC3"), ]
...
theme_set(
  ...
  legend.box = "vertical")+theme_void()
  )
